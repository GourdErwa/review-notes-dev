...
一般的做法是采用轮询方法，一直 while 循环（中间睡眠几豪秒）判断是否有数据。该办法可能存在的问题是
...
> 此处定义 **对象 O**、**等待的后续操作** 是关键理解点 ， 阅读本节内容后深入思考 **等待/通知的经典范式及应用场景** 一节

## JDK 提供方法
Object 作为所有对象的父类，其中与等待通知机制相关几个方法如下：

- notify : 通知一个在对象上等待的线程，使其从 wait 方法返回，前提是该线程获取到了对象锁
...
### 等待方遵循原则
1. 获取对象的锁
2. 如果条件（e.g. flag）不满足，那么调用对象的 wait 方法，被通知后仍要检查条件
3. 条件满足则执行对应的逻辑
...
```
## 应用场景
多线程执行时，线程内部逻辑需要等待其他线程执行后满足条件**才执行 wait 方法后需的逻辑**
