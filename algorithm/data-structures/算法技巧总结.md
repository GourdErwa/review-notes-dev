...
- **注意边界**
    ...
    一定考虑数据边界问题，尤其是 `>= > < <= null 空头、空尾...` 之类的问题，很多 BUG 都是因为边界处理不周全导致的。  
...
## 4. 树递归（自顶向下、自底向上）
[力扣-运用递归解决树的问题 ](https://leetcode-cn.com/explore/learn/card/data-structure-binary-tree/3/solve-problems-recursively/11/)

自顶向下：在递归层级中，我们根据节点计算出一些值，并在递归调用函数时将这些值传给子节点。（类似前序遍历）
```java
// 注意：方法参数的改变
...
```
自底向上：在每个递归层次上，我们首先对所有子节点递归地调用函数，然后根据返回值和根节点本身的值得到答案。（类似后序遍历）
```java
// 注意：返回值
